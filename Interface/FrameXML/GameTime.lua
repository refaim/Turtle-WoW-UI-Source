...
function GameTimeFrame_UpdateTooltip(hours, minutes)
	if (TwentyFourHourTime) then
		...
		GameTooltip:AddLine(GAMETIME_SERVER .. ": "..format(TEXT(TIME_TWENTYFOURHOURS), hours, minutes))

		local hours, minutes = date("%H"), date("%M")
		GameTooltip:AddLine(GAMETIME_LOCAL .. ": "..format(TEXT(TIME_TWENTYFOURHOURS), hours, minutes))
		GameTooltip:Show()
	else
        GameTooltip:ClearLines()
...
function GameTimeAMPM(hours,minutes)
	if not hours or minutes then return end
	local hours = tostring(hours)
