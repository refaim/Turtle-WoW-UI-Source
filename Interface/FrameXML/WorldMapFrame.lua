...
function WorldMapFrame_Minimize()
	...
	WorldMapPositioningGuide:SetPoint("BOTTOMRIGHT", WorldMapFrame, "BOTTOMRIGHT", 1, 0);
...
function WorldMapFrame_LoadContinents(...)
	...
	for i = 1, arg.n, 1 do
		...
		info.checked = UIDropDownMenu_GetSelectedID(WorldMapContinentDropDown) == i;
...
function WorldMapFrame_LoadZones(...)
	...
	for i = 1, arg.n, 1 do
		...
		info.checked = UIDropDownMenu_GetSelectedID(WorldMapZoneDropDown) == i;
		UIDropDownMenu_AddButton(info);
	end
end
...
function WorldMapZoomOutButton_OnClick()
	...
	if ( continentId > CONTINENTS_LENGTH or zoneId == 0 ) then
		SetMapZoom(0);
	else
...
function WorldMap_UpdateContinentDropDownText()
	local continentId = GetCurrentMapContinent();
	if ( continentId > CONTINENTS_LENGTH ) then
